...
export async function POST(
  ...
  await prisma.rating.upsert({
    where: { userId_spotId: { userId: session.user.id, spotId: id } },
    update: { value },
    create: { userId: session.user.id, spotId: id, value },
  });

  const ratings = await prisma.rating.findMany({ where: { spotId: id } });
